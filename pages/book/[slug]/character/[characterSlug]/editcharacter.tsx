import styles from "../../../../../styles/EditCharacter.module.css";
...
const NewCharacter = () => {
  ...
  // Etat pour la photo récupéré au backend
      ...
      .then((res) =>
        res.json().then((data) => {
          if (res.ok) {
            ...
            setFamilyRelation(data.family);
            setFears(data.fears);
            setHeight(data.height);
            setHometown(data.hometown);
            setTempPhoto(data.image);
  ...
  const handleSubmit = (): void => {
    ...
    fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/${slug}/updatecharacter/${characterSlug}/`,
      {
        method: "PATCH",
        body: formData,
      }
      ...
      .then((response) =>
        response.json().then((data) => {
          if (response.ok) {
            console.log("PERSO MODIFIEE ✏️✏️✏️", data);
            setPhoto(null);
            setFullName("");
            setSurname("");
            setRole("");
            ...
            setTalents([]);
            setBackground("");
            alert(`Votre personnage ${data.name} a bien été modifié.e`);
            router.push(`/book/${data.book}/character/${data.slug}`);
          }
        })
      )
      // .then(async (res) => {
